...
import {
  formatUnixTimestamp,
  formatUnixTimestampHM,
  formatUnixTimestampYMD,
  truncateString,
} from "../../common/Utils";
...
const Home = ({ navigation }) => {
    ...
    try {
      ...
      if (responseData && responseData.data) {
        ...
        const formattedMatches = matches.map((match) => ({
          ...
          winingPrice: match.winingPrice || "00",
        }));
  ...
  const fetchFeatureMatches = async () => {
    try {
      ...
      const responseData = await response.json();
      if (responseData && responseData.data) {
        setFeatruedMatches(responseData?.data);
      } else {
        console.error("Expected data structure not found in API response");
      }
    } catch (error) {
      ...
      <View style={styles.main}>
        ...
        <ScrollView style={{ padding: 5, margin: 5 }}>
          ...
          <View style={{ zIndex: 9999 }}>
            <Image
              source={require("../../assets/Images/ranvijay.png")}
              style={{
                resizeMode: "contain",
                position: "absolute",
                zIndex: 999,
              ...
              {bannerData.map((_, index) => (
                <View
                  ...
                  style={[
                    ...
                    {
                      backgroundColor:
                        index === currentPage ? "#FFFFFF" : "#000000",
                      width: index === currentPage ? 27 : 8,
                    },
                  ]}
                />
              ))}
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Text style={{ fontWeight: "400", fontSize: 14 }}>
                Featured{" "}
                <Text style={{ color: "#FF0F0F", fontWeight: "600" }}>
                  Matches
                </Text>
              </Text>
              <Image
                source={require("../../assets/Iocns/signals.png")}
                style={{ left: 10, width: 16, height: 11 }}
              />
            </View>
            <TouchableOpacity
            ...
            >
              <Image
                source={require("../../assets/Iocns/RightSideArrow.png")}
                style={{ right: 20, width: 10, height: 16 }}
              />
            </TouchableOpacity>
          </View>
          ...
          <ScrollView
            ...
            style={{ padding: 1, margin: 1, marginTop: 10 }}
          >
            {featruedMatches &&
              ...
              return (
                ...
                >
                  <UpCommingMatchesCard
                    ...
                    flagFirstTeam={
                      item?.teamsDetails?.teamOneDeatils?.flagFirstTeam ||
                      require("../../assets/Iocns/FlagPlaceholder.png")
                    }
                    teamsOneName={item?.teamA?.code || "-"}
                    flagSecondTeam={
                      item?.teamsDetails?.teamTwoDeatils?.flagSecondTeam ||
                      require("../../assets/Iocns/FlagPlaceholder.png")
                    }
                    teamsTwoName={item?.teamB?.code || "-"}
                    hour={
                      formatUnixTimestamp(item?.startAt, { date: true }) || "-"
                    }
                    min={
                      formatUnixTimestamp(item?.startAt, { time: true }) || "-"
                    }
                    batPrice={item.winingPrice}
                    onPress={() => {
